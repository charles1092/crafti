    ...
    <div className="space-y-3">
      {stages.map((stage, idx) => (
        <div key={idx} className={`flex items-center gap-3 p-3 rounded-lg transition-all ${
          idx === currentStage ? 'bg-slate-50 border-2 border-[#2A3A5C]' :
          idx < currentStage ? 'bg-gray-100 border-2 border-gray-300' :
          'bg-gray-50 border-2 border-gray-100'
        }`}>
          {idx < currentStage ? (
            <CheckCircle className="w-5 h-5 text-[#1B2541] flex-shrink-0" />
          ) : idx === currentStage ? (
            <Loader2 className="w-5 h-5 text-[#1B2541] animate-spin flex-shrink-0" />
          ) : (
